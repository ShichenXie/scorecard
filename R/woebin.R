...
woebin2 <- function(dt, y, x="", binnum=6, method="tree", min_perc_total=0.01, positive="bad|1", print_step = FALSE) {
  ...
  # input data.table
  dt <- data.table(dt)[,c(y,x),with=FALSE]
  ...
  breakpoints <- function(dtm, min_perc_total) {
    if ( is.numeric(dtm[,value]) | is.logical(dtm[,value]) ) {
      ...
      if (length(unique(xvec)) < n) {n <- length(unique(xvec))}

      # # digits for round
      # digits <- nchar(sub('^[0-9]+', '', median(xvec))) -1
      # if (digits < 2) digits = 2

      # breakpoints vector
      brkp <- quantile(xvec, (0:n)/n, na.rm=TRUE)
      brkp <- unique(c(-Inf, brkp[2:(length(brkp)-1)], Inf))
      # brkp <- round(brkp, digits)
      # brkp <- sprintf(paste0("%.", digits, "f"), brkp, digits)
...
# variable bstbin bstbrkp good bad badprob woe miv total_iv
  ...
  add_bst_brkp <- function(brkpdt, bst_brkp = c()) {
    ...
    bst_brkp_set <- setdiff( brkpdt[,brkp], c(bst_brkp, Inf, NA) )
    ...
    for (i in bst_brkp_set) {
      ...
      iv_dt <- rbindlist(list(
        iv_dt,
        data.table( iv = bst_brkpdt[,unique(total_iv)], brkp = i )
      ))
    }

    # selected best breakpoint
    bst_brkp <- sort( c(bst_brkp, iv_dt[iv==max(iv), brkp]) )
    #
  ...
  all_bst_brkp <- function(brkpdt, binnum=6, print_step=FALSE) {
    len_brkp <- length(setdiff(brkpdt[, brkp], c(Inf, NA)))
    if ( binnum > len_brkp ) binnum <- len_brkp-1
...
woebin <- function(dt, y, x="", binnum=6, method="tree", min_perc_total=0.01, positive="bad|1", print_step = FALSE) {
