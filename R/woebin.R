...
#' @param no_cores Number of CPU cores for parallel computation. Defaults NULL. If no_cores equals NULL, the no_cores will set as 1 if length of x variables less than 20, and will set as the number of all CPU cores if length of x variables large and equal than 20.
...
#' @param dt A data frame.
#' @param y Name of y variable.
#' @param all_var Logical, default TRUE. It means whether to adjust all variables' binning breaks.
...
#' @export
#'
woebin_adj = function(bins, dt, y, all_var = TRUE, count_distr_limit = 0.05) {
  # global variables or functions
  ...
  xs_len = length(xs)
  if (xs_len == 0) {
    cat("The binning breaks of all variables are perfect according to default setting.", "\n")
    break
  }
  i = 1
  breaks_list = NULL
